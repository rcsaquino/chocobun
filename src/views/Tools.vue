<template>
	<v-card class="animate__animated animate__fadeInDown animate__faster">
		...
		<BMICalc :open="bmiCalc">
			<template v-slot:toolbar-items>
				<v-btn icon @click="bmiCalc = false">
					<v-icon class="textColor--text">close</v-icon>
				</v-btn>
			</template>
		</BMICalc>
		...
		<!-- Fluid Calculator -->
		<FluidCalc :open="fluidCalc">
			<template v-slot:toolbar-items>
				<v-btn icon @click="fluidCalc = false">
					<v-icon class="textColor--text">close</v-icon>
				</v-btn>
			</template>
		</FluidCalc>
...
import FluidCalc from "@/components/chocobun_tools/FluidCalc.vue";

export default {
	mixins: [dialogHelper],
	components: { BMICalc, PregnancyCalc, ABG, Randomizer, FluidCalc },
	data: () => ({
		hashID: "Tools",
		tools: [
			{
